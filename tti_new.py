...
connection_mode = "usb"
#connention_mode = "ethernet"

power_supply = CPX400DP()
tty_usb = "/dev/ttyACM0"  # ls /dev/tty* 
...
def ttiopen():
    if connection_mode == "ethernet":
        ...
        if tty_usb is None:
            ...
            power_supply.connect(tty_usb)
            print("TTI is connected")
...
def set_PS_parametrs(U1, I1, U2, I2):
    if (connection_mode == "ethernet"): 
        ...
        power_supply.setVoltage(U1, channel=1)
        power_supply.setCurrent(I1, channel=1)
        power_supply.setVoltage(U2, channel=2)
        power_supply.setCurrent(I2, channel=2)
        power_supply.enableOutput(channel=1)
        power_supply.enableOutput(channel=2)
...
def poweroff():
    if (connection_mode == "ethernet"):
        ...
        power_supply.disableOutput(channel=2)
        power_supply.disconnect()
